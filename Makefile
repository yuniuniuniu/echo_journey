...
ifeq ($(OS),win32)
    ...
    TEST_COMMAND := set PYTHONPATH=$(PYTHONPATH) && poetry run pytest -c pyproject.toml --cov-report=html --cov=echo_journey tests/
else
	PYTHONPATH := `pwd`
    TEST_COMMAND := PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=echo_journey tests/
endif
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 echo_journey tests

check-safety:
	poetry check
	poetry run safety check --full-report
	poetry run bandit -ll --recursive echo_journey tests
